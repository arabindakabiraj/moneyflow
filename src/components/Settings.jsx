...
import { Target, LogOut, User, CheckCircle, Lock, Plus, Trash2, AlertTriangle, Pencil, X, ChevronRight, Camera, Shield, Tag, Wallet, Info, Moon, Sun, RefreshCw, Bell, TrendingUp, TrendingDown, Download, BarChart3, Calculator, Scissors, Share2, Fingerprint, Clock, ShieldAlert, Users, Heart, MessageSquare, Smartphone, FileText, UserX, Eye, EyeOff, AlertCircle, ArrowLeft } from 'lucide-react'
...
export default function Settings() {
  ...
  const menuGroups = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          label: 'Preferences',
          desc: 'Manage theme, savings goals, and categories',
          action: () => setSubTab('preferences')
        }
      ]
    },
    {
      ...
      items: [
        {
          ...
          label: 'Budgets & Setup',
          desc: 'Opening balance, GST, and category budgets',
          action: () => setSubTab('finance')
        },
        {
          icon: Shield,
          ...
          desc: 'Set app PIN code, auto-lock, and biometrics',
          action: () => setSubTab('security')
        }
      ]
    },
    {
      ...
      items: [
        {
          ...
          label: 'Export & Share',
          desc: 'Download CSV, import SMS, share summary',
          action: () => setSubTab('data')
        },
        {
          icon: Info,
          ...
          desc: 'Privacy policy, terms, and app details',
          action: () => setSubTab('about')
        }
      ]
    },
    {
      ...
      items: [
        {
          ...
          desc: 'Temporarily deactivate or delete account',
          action: () => setSubTab('danger')
        },
        {
          icon: LogOut,
  ...
  const renderSubTabHeader = (title) => (
    <div className="flex items-center gap-3 mb-6 pb-3 border-b border-black/[0.08] dark:border-white/[0.08] animate-fade-in">
      <button
        onClick={() => setSubTab('menu')}
        className="w-9 h-9 rounded-xl flex items-center justify-center bg-gray-150 dark:bg-[#222226] text-gray-500 dark:text-white/60 hover:bg-gray-200 dark:hover:bg-white/[0.04] active:scale-95 transition-all border-none cursor-pointer"
      >
        <ArrowLeft size={16} />
      </button>
      <div>
        <h2 className="text-base font-bold text-gray-900 dark:text-white/95 leading-tight">{title}</h2>
        <p className="text-[10px] text-gray-400 dark:text-white/35 font-medium uppercase tracking-wider mt-0.5">Settings / {title}</p>
      </div>
    </div>
  )
    ...
    <div className="space-y-6 animate-fade-in pb-8">
          ...
          <div className="space-y-4">
            {menuGroups.map((group, gIdx) => (
              <div key={gIdx} className="space-y-2">
                <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-white/35 px-1">
                  {group.title}
                </p>
                <div className="rounded-2xl bg-white dark:bg-[#1A1A1D] border border-black/[0.08] dark:border-white/[0.08] overflow-hidden divide-y divide-black/[0.06] dark:divide-white/[0.06] shadow-sm">
                  {group.items.map((item, iIdx) => (
                    <button
                      key={iIdx}
                      onClick={item.action}
                      className="w-full flex items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-white/[0.02] active:bg-gray-100 dark:active:bg-white/[0.04] transition-all border-none bg-transparent cursor-pointer text-left"
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-2xl ${item.iconBg} flex items-center justify-center shrink-0`}>
                          <item.icon size={18} className={item.iconColor} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-gray-900 dark:text-white/95">{item.label}</p>
                          {item.desc && <p className="text-[11px] text-gray-400 dark:text-white/35 mt-0.5">{item.desc}</p>}
                        </div>
                      </div>
                      <ChevronRight size={16} className="text-gray-300 dark:text-white/20 shrink-0 ml-2" />
                    </button>
                  ))}
                </div>
              </div>
      ...
      {subTab === 'profile' && (
        <div className="animate-fade-in space-y-4">
          {renderSubTabHeader('Personal Details')}
          <div className="rounded-2xl bg-white dark:bg-[#1A1A1D] border border-black/[0.08] dark:border-white/[0.08] p-5 space-y-4">
            <div>
      ...
      {subTab === 'preferences' && (
        <div className="animate-fade-in space-y-4">
          {renderSubTabHeader('Preferences')}
          <div className="rounded-2xl bg-white dark:bg-[#1A1A1D] border border-black/[0.08] dark:border-white/[0.08] overflow-hidden divide-y divide-black/[0.06] dark:divide-white/[0.06] shadow-sm">
            ...
            <div className="px-4 py-4 flex items-center gap-3">
              ...
              <button
                ...
                className={`relative w-14 h-7 rounded-full transition-all duration-300 border-none cursor-pointer ${darkMode ? 'bg-violet-500' : 'bg-amber-400'}`}
              >
            ...
            <div className="flex items-center gap-3 px-4 py-4">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <input type="number" value={goalInput} onChange={e => setGoalInput(e.target.value)}
                  className="w-20 text-right text-sm font-mono font-semibold text-[#4F8EF7] bg-transparent outline-none border-none focus:ring-0" />
                <button onClick={saveGoal}
                  className={`ml-1 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all border-none cursor-pointer ${saved ? 'bg-[#4F8EF7] text-white scale-95' : 'bg-[#4F8EF7]/10 text-[#4F8EF7] hover:bg-brand-100 active:scale-95'}`}>
                  {saved ? '✓' : 'Save'}
                </button>
              </div>
            </div>
            ...
            <div>
              <button onClick={() => setExpandCategories(v => !v)}
                className="w-full flex items-center gap-3 px-4 py-4 hover:bg-gray-50 dark:hover:bg-white/[0.02] active:bg-white/[0.04] transition-colors border-none bg-transparent cursor-pointer">
                ...
                <ChevronRight size={16} className={`text-gray-300 dark:text-white/20 transition-transform duration-200 shrink-0 ${expandCategories ? 'rotate-90' : ''}`} />
              </button>
              {expandCategories && (
                <div className="px-4 pb-4 animate-fade-in">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => { if (newCat.trim()) { addCategory(newCat.trim()); setNewCat('') } }}
                      className="w-10 h-10 rounded-xl bg-[#4F8EF7] text-white flex items-center justify-center shrink-0 active:scale-95 transition-transform shadow-sm shadow-[#4F8EF7]/20 border-none cursor-pointer">
                      <Plus size={16} />
                    </button>
                  </div>
                </div>
      ...
      {/* ── SubTab: Budgets and Setup ── */}
      {subTab === 'finance' && (
        <div className="animate-fade-in space-y-6">
          {renderSubTabHeader('Budgets & Financial Setup')}

          {/* Financial Setup Card */}
          <div className="space-y-2">
            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-white/35 px-1">Financial Options</p>
            <div className="rounded-2xl bg-white dark:bg-[#1A1A1D] border border-black/[0.08] dark:border-white/[0.08] overflow-hidden divide-y divide-black/[0.06] dark:divide-white/[0.06] shadow-sm">
              {/* Opening Balance */}
              <div className="px-4 py-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  {openingBalance > 0 && (
                    <span className="text-[10px] font-bold px-2.5 py-1 rounded-lg bg-[#34D399]/15 text-[#34D399] shrink-0">
                ...
                <div className="space-y-2">
                  <input
                    ...
                    className="w-full px-3 py-2.5 rounded-xl bg-gray-100 dark:bg-[#222226] border border-black/[0.08] dark:border-white/[0.08] text-sm font-mono text-gray-900 dark:text-white/95 outline-none focus:ring-2 focus:ring-[#4F8EF7]/50"
                  />
                  <div className="flex gap-2 items-center">
                    ...
                    <button
                      ...
                      className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all active:scale-[0.98] shrink-0 border-none cursor-pointer ${obSaved ? 'bg-emerald-500 text-white' : 'bg-[#4F8EF7] text-white hover:bg-[#4F8EF7]/80'}`}>
                      {obSaved ? '✓ Saved' : 'Save'}
                    </button>
                  </div>
                </div>
              ...
              <div className="px-4 py-4 flex items-center justify-between gap-3">
                ...
                <button
                  ...
                  className={`relative w-12 h-7 rounded-full transition-all duration-300 border-none cursor-pointer ${gstSettings.registered ? 'bg-[#4F8EF7]' : 'bg-white/[0.06]'}`}>
              ...
              <div className="px-4 py-3.5">
                <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-white/30 mb-2">Accounting Shortcuts</p>
                <div className="flex gap-2">
                  <button onClick={() => setActiveTab('ledger')}
                    className="flex-1 flex items-center justify-center gap-1.5 text-[11px] font-semibold text-[#4F8EF7] bg-indigo-50 dark:bg-indigo-900/20 py-2.5 rounded-xl hover:bg-[#4F8EF7]/15 transition-colors border-none cursor-pointer">
                    📒 Ledger
                  </button>
                  <button onClick={() => setActiveTab('cashflow')}
                    className="flex-1 flex items-center justify-center gap-1.5 text-[11px] font-semibold text-[#34D399] bg-emerald-50 dark:bg-emerald-900/20 py-2.5 rounded-xl hover:bg-[#34D399]/15 transition-colors border-none cursor-pointer">
                    💵 Cash Flow
                  </button>
                  <button onClick={() => setActiveTab('groups')}
                    className="flex-1 flex items-center justify-center gap-1.5 text-[11px] font-semibold text-[#4F8EF7] bg-violet-500/10 py-2.5 rounded-xl hover:bg-violet-500/12 transition-colors border-none cursor-pointer">
                    👥 Groups
                  </button>
                </div>
              </div>
          ...
          {/* Budget Limits Card */}
          <div className="space-y-2">
            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-white/35 px-1">Budget Limits</p>
            <div className="rounded-2xl bg-white dark:bg-[#1A1A1D] border border-black/[0.08] dark:border-white/[0.08] p-4 shadow-sm space-y-4">
              {Object.keys(budgets).length > 0 ? (
                    ...
                    <div key={cat} className="flex items-center justify-between py-1.5 px-3 rounded-xl bg-gray-100 dark:bg-[#222226]/50">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => removeBudget(cat)}
                          className="w-7 h-7 rounded-lg bg-rose-50 dark:bg-rose-900/20 flex items-center justify-center text-rose-400 hover:text-[#FF6B6B] active:scale-90 transition-all border-none cursor-pointer">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </div>
                ...
                <p className="text-xs text-gray-400 dark:text-white/35 text-center py-2">Set budget limits to track spending by category</p>
              )}
              <div className="flex gap-2">
      ...
      {/* ── SubTab: Security and lock ── */}
      {subTab === 'security' && (
        <div className="animate-fade-in space-y-4">
          {renderSubTabHeader('Security & App Lock')}
          <div className="rounded-2xl bg-white dark:bg-[#1A1A1D] border border-black/[0.08] dark:border-white/[0.08] overflow-hidden divide-y divide-black/[0.06] dark:divide-white/[0.06] shadow-sm">
            {/* PIN Code protection */}
            <div className="px-4 py-4">
              <div className="flex items-center gap-3 mb-4">
                ...
                {isPinSet() && (
                  <span className="px-2.5 py-1 rounded-lg bg-[#34D399]/15 text-[#34D399] text-[10px] font-bold uppercase tracking-wide shrink-0">Active</span>
                )}
              </div>
              ...
              {pinStep === 'idle' ? (
                <div className="flex gap-2 animate-fade-in">
                  <button onClick={() => setPinStep('setup')}
                    className="flex-1 py-2.5 rounded-xl text-xs font-bold bg-violet-500/10 text-violet-700 dark:text-violet-300 border border-violet-500/20 hover:bg-violet-500/15 transition-colors active:scale-[0.98] cursor-pointer">
                    {isPinSet() ? 'Change PIN' : 'Set PIN'}
                  </button>
                  {isPinSet() && (
                    <button onClick={() => { clearPin(); setPinMsg('🔓 PIN removed') }}
                      className="px-5 py-2.5 rounded-xl text-xs font-bold bg-rose-50 dark:bg-rose-900/20 text-[#FF6B6B] border border-[#FF6B6B]/20 hover:bg-[#FF6B6B]/15 transition-colors active:scale-[0.98] cursor-pointer">
                      Remove
                    </button>
                  )}
                </div>
                ...
                <div className="space-y-3 animate-fade-in">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setPinStep('idle'); setPin1(''); setPin2(''); setPinMsg('') }}
                      className="flex-1 py-2.5 rounded-xl text-xs font-bold bg-gray-100 dark:bg-[#222226] text-gray-400 dark:text-white/40 active:scale-[0.98] transition-transform border-none cursor-pointer">Cancel</button>
                    <button onClick={handleSetPin}
                      className="flex-1 py-2.5 rounded-xl text-xs font-bold bg-violet-500 text-white active:scale-[0.98] transition-transform shadow-sm shadow-violet-500/20 border-none cursor-pointer">Set PIN</button>
                  </div>
                </div>
              )}
            </div>
            ...
            {bioCapable && isPinSet() && (
              <div className="px-4 py-4 flex items-center justify-between gap-3">
                ...
                <button
                  ...
                  className={`relative w-12 h-7 rounded-full transition-colors duration-200 border-none cursor-pointer ${bioEnabled ? 'bg-[#4F8EF7]' : 'bg-white/[0.06]'} ${bioLoading ? 'opacity-50' : ''}`}>
                  <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow-sm transition-transform duration-200 ${bioEnabled ? 'translate-x-6' : 'translate-x-0'}`} />
                </button>
              </div>
            )}
      ...
      {subTab === 'data' && (
        <div className="animate-fade-in space-y-4">
          {renderSubTabHeader('Export & Tools')}
          <div className="rounded-2xl bg-white dark:bg-[#1A1A1D] border border-black/[0.08] dark:border-white/[0.08] overflow-hidden divide-y divide-black/[0.06] dark:divide-white/[0.06] shadow-sm">
            <SettingsRow
      ...
      {subTab === 'about' && (
        <div className="animate-fade-in space-y-4">
          {renderSubTabHeader('About & Policies')}
          <div className="rounded-2xl bg-white dark:bg-[#1A1A1D] border border-black/[0.08] dark:border-white/[0.08] overflow-hidden divide-y divide-black/[0.06] dark:divide-white/[0.06] shadow-sm">
            <div className="flex items-center gap-3 px-4 py-4">
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-[#4F8EF7] to-emerald-500 flex items-center justify-center shrink-0 shadow-md shadow-[#4F8EF7]/20">
          ...
          <div className="w-full max-w-sm mx-4 mb-6 sm:mb-0 rounded-3xl shadow-2xl overflow-hidden animate-slide-up" style={{ background: 'var(--mf-surface)', border: '1px solid var(--mf-border)' }}
            ...
            <div className="px-6 pb-6 flex gap-3">
              ...
              <button onClick={() => { clearPin(); clearBiometric(); setAutoLockMinutes(0); setShowLogoutModal(false); logout() }}
                className="flex-1 py-3 rounded-2xl text-sm font-semibold bg-rose-500 text-white active:scale-[0.98] transition-transform shadow-sm shadow-red-500/20 border-none cursor-pointer">
                Log Out
              </button>
            </div>
          </div>
